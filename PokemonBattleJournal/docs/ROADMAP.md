...
| F-32 | Deck comparer | Side-by-side diff of two lists, previously only in the memory roadmap. `Services/Restore/NoteDiff` gives the generic line diff for free, but a good comparer needs a **card-aware** diff on top: `4 Iono` against `2 Iono` should read as a count change on one card, not a removal plus an addition. So the line diff is the fallback, not the feature. Sizing is settled and MEASURED — `NoteDiff.MaxLines` is **300**, lowered from 1000 on 2026-08-09 after benchmarking: at 1000 a diff cost 8.9ms and forced a full Gen2 collection on essentially every call (4MB table on the Large Object Heap), while the fallback past the bound was 112x cheaper than the diff it replaced. A list is ~30 lines, so 300 is ample. |
...
| F-33 | Accessibility contract tests for Android | The contract is **Windows-only today, and that is a real second-platform hole rather than bookkeeping**: ~18 of the 22 Windows-only test cases are `MainPage_InteractiveElement_IsAnnouncedAndOperable` and its OptionsPage twin, which read the live **UIA** tree and require every interactive element to have both a Name and a control pattern. UIA is a Windows API; Android's equivalent is `AccessibilityNodeInfo`, a different tree with different semantics, so these need rewriting rather than recompiling. **Consequence: TalkBack is unverified.** Everything known about this app's accessibility comes from Windows, and the defect that motivated the contract — the BO3 switch, announced correctly and impossible to activate — was only catchable because Windows exposes patterns. An Android-only version of that bug ships silently today. The Android analogue would assert, per element, a non-empty `content-desc` **or** `text` (announced) plus `clickable="true"` (operable); UiAutomator2 exposes both. **One known complication:** MAUI resets `content-desc` from the AutomationId to `SemanticProperties.Description` on a native re-render, so an assertion keyed on either alone is unstable after a binding update — see `feedback_maui_content_desc_reset`. Note also that elements only visible in a transient state (the restore conflict buttons) cannot join a page-level contract test on either platform, and carry their own assertions in the feature's own test. |
| F-34 | Deck builder, on card data from the TcgDex.CSharpSdk NuGet package | User, 2026-08-09. Take the SDK as a dependency rather than scraping or hand-maintaining a card database — it is the user's own package, so the maintenance question is already answered, and it is fuzz-tested and mutation-tested in its own repo. **It unblocks more than the builder.** F-32 (deck comparer) needs a **card-aware** diff — `4 Iono` against `2 Iono` should read as a count change on one card, not a removal plus an addition — and card-aware means having card identity, which is exactly what the SDK supplies. F-31 (paste a deck list into a note) becomes parseable rather than free text for the same reason. So the ordering is: SDK first, then builder, comparer and list-parsing all get cheaper. **Two constraints carry over.** The SDK talks to `api.tcgdex.net`, so this is the second network dependency after Limitless — it needs the same treatment: bounded response size, failure that degrades rather than blocks, and nothing about a user's decks leaving the device (a card lookup is a public query; a *deck* is user content). And card data is untrusted input from a server this app does not control, so it belongs behind the same size/depth limits as the TrainerHill import — see `Services/Import/TrainerHillImportService`. |
