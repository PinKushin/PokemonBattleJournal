namespace PokemonBattleJournal.Services
{
    public class BO3ResultCalculator : IMatchResultCalculator
    {
        public MatchResult CalculateResult(MatchResult? result1, MatchResult? result2 = null, MatchResult? result3 = null)
        {
            ...
            if (result1 == MatchResult.Win) wins++;
            if (result1 == MatchResult.Loss) losses++;

            if (result2 == MatchResult.Win) wins++;
            if (result2 == MatchResult.Loss) losses++;

            if (result3 == MatchResult.Win) wins++;
            if (result3 == MatchResult.Loss) losses++;
            ...
            if (wins > losses) return MatchResult.Win;
            if (losses > wins) return MatchResult.Loss;
